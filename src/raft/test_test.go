...
func TestReElection2A(t *testing.T) {
	...
	fmt.Println("[TESTS] first check")
	leader1 := cfg.checkOneLeader()

	// if the leader disconnects, a new one should be elected.
	cfg.disconnect(leader1)
	fmt.Printf("[TESTS] disconnected leader %v\n", leader1)
	cfg.checkOneLeader()
	fmt.Println("[TESTS] reached 1")
	...
	fmt.Printf("[TESTS] disconnected %v and %v\n", leader2, (leader2+1)%servers)
	time.Sleep(2 * RaftElectionTimeout)
	...
	fmt.Printf("[TESTS] reconnected server %v\n", (leader2+1)%servers)
	cfg.checkOneLeader()
	fmt.Println("[TESTS] reached 3")

	// re-join of last node shouldn't prevent leader from existing.
	cfg.connect(leader2)
	cfg.checkOneLeader()
	fmt.Println("[TESTS] reached 4")
	cfg.end()
}
